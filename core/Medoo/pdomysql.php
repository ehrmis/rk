...
//提供pdo_mysql引擎管理数据库各种方法	
$db = new PdoMysql($configArr);   
class PdoMysql
{
...
//*******************20170518************************************
    ...
    */
     ...
     * @return int 受影响的行数
     */
    public function delete($tbName,$_where) {
        $sql = "delete from ".$tbName." ".$_where;
        $this->_clear = 1;
        $this->_clear();
        return $this->_doExec($sql);
    }
     ...
     */
    //法一：单一字段更新*******20170518************
  /*  public function update ( $tbName, $data, $_where ) {
        $sql = '';
        $sql .= "UPDATE $tbName SET " . substr( $this->implode_fields( $data ), 0, -1 )
            . " " . $_where;
        return $this->_doExec($sql);
    }*/
   ...
   public function update($tbName,$data,$_where) {
        ...
        $sql = "update ".trim($tbName)." set ".trim($valStr)." ". $_where;
